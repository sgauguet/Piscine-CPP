...
FragTrap::FragTrap(std::string name) : ClapTrap(name, 100, 100, 100, 100, 1, 30, 20, 5), _shout_attack(5), _cyber_attack(35), _mars_attack(55) {
...
void	FragTrap::intimidatingShout(std::string const & target) {
	std::cout << this->_name << " raised an intimidating shout ; " << target << " has suffered " << this->_shout_attack << " points of damage !" << std::endl;
}

void	FragTrap::cyberAttack(std::string const & target) {
	std::cout << this->_name << " launches cyber attack on " << target << ", causing " << this->_cyber_attack << " points of damage !" << std::endl;
}

void	FragTrap::marsAttack(std::string const & target) {
	std::cout << this->_name << " attacks " << target << " from Mars, causing " << this->_mars_attack << " points of damage !" << std::endl;
}

void	FragTrap::rangedAttack(std::string const & target) {
	std::cout << this->_name << " attacks " << target << " at range, causing " << this->_ranged_attack << " points of damage ! (fragtrap)" << std::endl;
}

void	FragTrap::meleeAttack(std::string const & target) {
	std::cout << this->_name << " attacks " << target << " at melee, causing " << this->_melee_attack << " points of damage ! (fragtrap)" << std::endl;
}
